...
for record in lrdwws_uncontrol_train_data:
    text = record["text"]
...
#model_path = Path("./checkpoints/train_hybrid/finetuned_asr_model.nemo"

# Paths to your .yaml and .ckpt files
#yaml_path = './checkpoints/finetuned_asr_model/model_config.yaml'
#ckpt_path = './checkpoints/finetuned_asr_model//model_weights.ckpt'
...
config.model.validation_ds.manifest_filepath = str(LRDWWS_DYSARTHRIC_DATASET / "manifest_test/test.json")
...
config.exp_manager.wandb_logger_kwargs = {
    ...
    "name": "lrdwws_dyarthric_independent",
}
...
hybrid_model.save_to("checkpoints/train_hybrid/finetuned_hybrid_model.nemo")
# extract the resulting ASR model from the hybrid model
hybrid_model.save_asr_model_to("checkpoints/train_hybrid/finetuned_asr_model.nemo") #checkpoints/train_hybrid/finetuned_asr_model.nemo
